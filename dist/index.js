...
var _parseToEntries = __nccwpck_require__(9368);
...
var _parseToProperties = __nccwpck_require__(9958);
...
var _stringify = __nccwpck_require__(9721);
...
var PropertiesParser = _interopRequireWildcard(__nccwpck_require__(8717));

var _options = __nccwpck_require__(8170);
...
//# sourceMappingURL=properties.pegjs.js.map
...
const getCommit = async (sha) => {
    ...
    const response = await (0, fetch_1.get)(url);
    return response;
};
...
const properties_1 = __importDefault(__nccwpck_require__(9957));
/**
...
async function run() {
    try {
        ...
        const propertiesFileStr = (0, fs_1.readFileSync)((0, path_1.join)(dir, fileName), {
            encoding: 'utf8'
        });
        const propertiesFile = properties_1.default.parseToEntries(propertiesFileStr);
...
const checkDiff = (commit, fileName, property, version) => {
    const file = commit.files.find(fileObj => fileName.includes(fileObj.filename));
    const rawLines = file?.patch
        .split('\n')
        .filter(line => line.includes(`${property} =`) && ['+', '-'].includes(line[0]));
    if (!rawLines) {
...
function matchVersion(str) {
    const semverRegex = /(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(-(0|[1-9A-Za-z-][0-9A-Za-z-]*)(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?/g;
